...
JSFLAGS += -s USE_SDL=2 -s WASM=0 -s ASYNCIFY=1 -s 'ASYNCIFY_IMPORTS=["mp_js_sleep_with_intr"]'
...
$(BUILD)/micropython.js: $(OBJ) library.js wrapper.js
	...
	cp $@ $(BUILD)/../lvgl_mp.js
	([ -e $(BUILD)/firmware.wasm ] && cp $(BUILD)/firmware.wasm $(BUILD)/../firmware.wasm); true
