 ...
 *
 * Command line:
 * ../../lib/lv_bindings/gen/gen_mpy.py -M lodepng -E build/lodepng/lodepng.pp.c ../../lib/lv_bindings/driver/png/lodepng/lodepng.h
 *
...
#include "../../lib/lv_bindings/driver/png/lodepng/lodepng.h"
...
STATIC const mp_obj_type_t mp_LCT_type = {
    { &mp_type_type },
    .name = MP_QSTR_LCT,
    .print = LCT_print,
...
STATIC inline mp_obj_t mp_read_ptr_LodePNGDecompressSettings(struct LodePNGDecompressSettings *field)
{
    return lv_to_mp_struct(get_mp_LodePNGDecompressSettings_type(), (void*)field);
}
...
STATIC inline mp_obj_t mp_read_ptr_LodePNGColorMode(LodePNGColorMode *field)
{
    return lv_to_mp_struct(get_mp_LodePNGColorMode_type(), (void*)field);
}
...
STATIC void mp_LodePNGColorMode_attr(mp_obj_t self_in, qstr attr, mp_obj_t *dest)
{
    ...
    if (dest[0] == MP_OBJ_NULL) {
        ...
        switch(attr)
        {
            ...
            case MP_QSTR_palette: dest[0] = ptr_to_mp((void*)data->palette); break; // converting from unsigned char *;
        ...
        if (dest[1])
        {
            ...
            switch(attr)
            {
                ...
                case MP_QSTR_palette: data->palette = (void*)mp_to_ptr(dest[1]); break; // converting to unsigned char *;
...
STATIC inline mp_obj_t mp_read_ptr_LodePNGTime(LodePNGTime *field)
{
    return lv_to_mp_struct(get_mp_LodePNGTime_type(), (void*)field);
}
...
STATIC unsigned char * *mp_arr_to_unsigned_char_ptr__3__(mp_obj_t mp_arr)
{
    mp_obj_t mp_len = mp_obj_len_maybe(mp_arr);
    if (mp_len == MP_OBJ_NULL) return mp_to_ptr(mp_arr);
    mp_int_t len = mp_obj_get_int(mp_len);
...
STATIC mp_obj_t mp_arr_from_unsigned_char_ptr__3__(unsigned char * *arr)
{
    mp_obj_t obj_arr[3];
    for (size_t i=0; i<3; i++){
        obj_arr[i] = ptr_to_mp(arr[i]);
    }
...
STATIC size_t *mp_arr_to_size_t___3__(mp_obj_t mp_arr)
{
    mp_obj_t mp_len = mp_obj_len_maybe(mp_arr);
    if (mp_len == MP_OBJ_NULL) return mp_to_ptr(mp_arr);
    mp_int_t len = mp_obj_get_int(mp_len);
...
STATIC mp_obj_t mp_arr_from_size_t___3__(size_t *arr)
{
    mp_obj_t obj_arr[3];
    for (size_t i=0; i<3; i++){
        obj_arr[i] = mp_obj_new_int_from_uint(arr[i]);
...
STATIC inline mp_obj_t mp_read_ptr_LodePNGInfo(LodePNGInfo *field)
{
    return lv_to_mp_struct(get_mp_LodePNGInfo_type(), (void*)field);
}
...
STATIC void mp_LodePNGInfo_attr(mp_obj_t self_in, qstr attr, mp_obj_t *dest)
{
    ...
    if (dest[0] == MP_OBJ_NULL) {
        ...
        switch(attr)
        {
            ...
            case MP_QSTR_text_keys: dest[0] = ptr_to_mp((void*)data->text_keys); break; // converting from char **;
            case MP_QSTR_text_strings: dest[0] = ptr_to_mp((void*)data->text_strings); break; // converting from char **;
            case MP_QSTR_itext_num: dest[0] = mp_obj_new_int_from_uint(data->itext_num); break; // converting from size_t;
            case MP_QSTR_itext_keys: dest[0] = ptr_to_mp((void*)data->itext_keys); break; // converting from char **;
            case MP_QSTR_itext_langtags: dest[0] = ptr_to_mp((void*)data->itext_langtags); break; // converting from char **;
            case MP_QSTR_itext_transkeys: dest[0] = ptr_to_mp((void*)data->itext_transkeys); break; // converting from char **;
            case MP_QSTR_itext_strings: dest[0] = ptr_to_mp((void*)data->itext_strings); break; // converting from char **;
            ...
            case MP_QSTR_iccp_profile: dest[0] = ptr_to_mp((void*)data->iccp_profile); break; // converting from unsigned char *;
        ...
        if (dest[1])
        {
            ...
            switch(attr)
            {
                ...
                case MP_QSTR_text_keys: data->text_keys = (void*)mp_to_ptr(dest[1]); break; // converting to char **;
                case MP_QSTR_text_strings: data->text_strings = (void*)mp_to_ptr(dest[1]); break; // converting to char **;
                case MP_QSTR_itext_num: data->itext_num = (size_t)mp_obj_get_int(dest[1]); break; // converting to size_t;
                case MP_QSTR_itext_keys: data->itext_keys = (void*)mp_to_ptr(dest[1]); break; // converting to char **;
                case MP_QSTR_itext_langtags: data->itext_langtags = (void*)mp_to_ptr(dest[1]); break; // converting to char **;
                case MP_QSTR_itext_transkeys: data->itext_transkeys = (void*)mp_to_ptr(dest[1]); break; // converting to char **;
                case MP_QSTR_itext_strings: data->itext_strings = (void*)mp_to_ptr(dest[1]); break; // converting to char **;
                ...
                case MP_QSTR_iccp_profile: data->iccp_profile = (void*)mp_to_ptr(dest[1]); break; // converting to unsigned char *;
                case MP_QSTR_iccp_profile_size: data->iccp_profile_size = (unsigned)mp_obj_get_int(dest[1]); break; // converting to unsigned;
                case MP_QSTR_unknown_chunks_data: memcpy(&data->unknown_chunks_data, mp_arr_to_unsigned_char_ptr__3__(dest[1]), sizeof(unsigned char *)*3); break; // converting to unsigned char *[3];
                case MP_QSTR_unknown_chunks_size: memcpy(&data->unknown_chunks_size, mp_arr_to_size_t___3__(dest[1]), sizeof(size_t)*3); break; // converting to size_t [3];
                default: return;
            }
...
STATIC inline mp_obj_t mp_read_ptr_LodePNGDecoderSettings(LodePNGDecoderSettings *field)
{
    return lv_to_mp_struct(get_mp_LodePNGDecoderSettings_type(), (void*)field);
}
...
STATIC inline mp_obj_t mp_read_ptr_LodePNGState(LodePNGState *field)
{
    return lv_to_mp_struct(get_mp_LodePNGState_type(), (void*)field);
}
...
STATIC char *mp_arr_to_char___5__(mp_obj_t mp_arr)
{
    mp_obj_t mp_len = mp_obj_len_maybe(mp_arr);
    if (mp_len == MP_OBJ_NULL) return mp_to_ptr(mp_arr);
    mp_int_t len = mp_obj_get_int(mp_len);
...
STATIC mp_obj_t mp_lodepng_convert(size_t mp_n_args, const mp_obj_t *mp_args, void *lv_func_ptr)
{
    ...
    const LodePNGColorMode *mode_out = mp_write_ptr_LodePNGColorMode(mp_args[2]);
...
STATIC const mp_rom_map_elem_t lodepng_globals_table[] = {
    ...
    { MP_ROM_QSTR(MP_QSTR_LCT), MP_ROM_PTR(&mp_LCT_type) },
